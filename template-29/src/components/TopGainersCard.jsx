...
export default function TopGainersCard() {
  return (
    <div className="bg-white dark:bg-slate-800/90 rounded-2xl p-5 border border-slate-200/80 dark:border-slate-700/80 shadow-sm flex flex-col justify-between h-[360px]">
      ...
      {/* Table Headers */}
      <div className="grid grid-cols-4 text-[11px] font-bold text-slate-400 uppercase tracking-wider py-1 text-left">
        <div className="flex items-center gap-1 cursor-pointer hover:text-slate-600">
          <span>Coin</span>
          <ArrowUpDown className="w-3 h-3" />
        </div>
        <div className="flex items-center gap-1 cursor-pointer hover:text-slate-600 text-center">
          <span>Cap</span>
          <ArrowUpDown className="w-3 h-3" />
        </div>
        <div className="flex items-center gap-1 cursor-pointer hover:text-slate-600 text-center">
      ...
      <div className="space-y-3 flex-1 flex flex-col justify-around py-1">
        {topGainers.map((gainer) => (
          <div key={gainer.id} className="grid grid-cols-4 items-center group cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/40 p-1.5 rounded-xl transition-colors">
            ...
            <div className="flex items-center gap-2">
              {gainer.icon === 'star' ? (
                ...
                <Zap className="w-4 h-4 text-[#FF5B37] fill-[#FF5B37]" />
              )}
