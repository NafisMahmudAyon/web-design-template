...
import { STATUS_GROUPS } from '../data/workspaceData';

export default function TaskCalendarView({ tasks, onSelectTask }) {
  const [currentMonth] = useState('December 2026');

  // Days of December 2026 (Dec 1, 2026 is Tuesday)
  // Calendar grid: Sun, Mon, Tue, Wed, Thu, Fri, Sat
  const daysOfWeek = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  
  // 31 days in Dec 2026. Tuesday is index 2. 2 empty padding slots for Sun & Mon.
  const paddingDays = [null, null];
  const daysInMonth = Array.from({ length: 31 }, (_, i) => i + 1);
  const calendarCells = [...paddingDays, ...daysInMonth];

  // Helper to match tasks with a day
  const getTasksForDay = (day) => {
    if (!day) return [];
    const formattedDay = day < 10 ? `0${day}` : `${day}`;
    const pattern = `${formattedDay} Dec 2026`;
    return tasks.filter(t => t.dueDate === pattern || t.start === pattern);
  };
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-semibold ${day === 15 ? 'w-5 h-5 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold' : 'text-slate-700'}`}>
                    {day}
                  </span>
                  {dayTasks.length > 0 && (
