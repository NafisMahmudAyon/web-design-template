...
import { KANBAN_TASKS, NOTIFICATIONS } from './data/kanbanData';

export default function App() {
  const [activeProject, setActiveProject] = useState('Product Launch 2026');
  const [currentView, setCurrentView] = useState('kanban'); // 'kanban' is active by default in template-40!
    ...
    setTasks(prev => prev.map(t => {
      if (t.id === taskId) {
        ...
        return {
          ...t,
          status: nextStatus,
          milestoneCompleted: nextStatus === 'completed' ? t.milestoneTotal : 0,
        };
      }
      return t;
    }));
    ...
    setTasks(prev => prev.map(t => {
      if (t.id === taskId) {
        return {
          ...t,
          status: newStatus,
          milestoneCompleted: newStatus === 'completed' ? t.milestoneTotal : t.milestoneCompleted,
        };
      }
      return t;
    }));
  ...
  return (
    <div className="min-h-screen bg-[#EBECEF] p-0 sm:p-4 md:p-6 lg:p-8 flex items-center justify-center font-sans antialiased text-slate-800">
      {/* Outer Workspace Card Frame matching Figma Presentation */}
